(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["ManhattanContent"] = factory();
	else
		root["ManhattanContent"] = factory();
})(this, function() {
...
var $, ContentFlow, ContentTools, FlowAPI, ImageUploader, SaveSemaphore, addSaveProcess, init;

$ = __webpack_require__(6);

ContentTools = __webpack_require__(2);

ContentFlow = __webpack_require__(3);

FlowAPI = __webpack_require__(4).FlowAPI;

ImageUploader = __webpack_require__(5).ImageUploader;

SaveSemaphore = (function() {
  function SaveSemaphore() {}

  SaveSemaphore._saving = 0;

  SaveSemaphore.dec = function() {
    return this._saving = Math.max(0, this._saving - 1);
  };

  SaveSemaphore.inc = function() {
    return this._saving += 1;
  };

  SaveSemaphore.saving = function() {
    return this._saving > 0;
  };

  return SaveSemaphore;

})();

addSaveProcess = function(url, params, buildContentsFunc) {
  var editor;
  editor = ContentTools.EditorApp.get();
  return editor.addEventListener('saved', function(ev) {
    var contents, formData, k, ref, regions, v, xhr;
    ...
    xhr.send(formData);
    return xhr.addEventListener('load', function(ev) {
      SaveSemaphore.dec();
      if (!SaveSemaphore.saving()) {
        editor.busy(false);
      }
...
init = function(baseURL, baseFlowURL, baseParams) {
  var api, editor, flowMgr, updateFlowsURL, updateGlobalURL;
  if (baseURL == null) {
    baseURL = '/';
  }
  if (baseFlowURL == null) {
  ...
  addSaveProcess(updateFlowsURL, baseParams, function(regions) {
    var _, contents, k, ref, region_name, snippetId, v;
    contents = {};
    for (k in regions) {
      v = regions[k];
      ...
      ref = k.split(':'), _ = ref[0], snippetId = ref[1], region_name = ref[2];
  ...
  return addSaveProcess(updateGlobalURL, baseParams, function(regions) {
    var _, contents, k, ref, region_name, v;
    contents = {};
    for (k in regions) {
      v = regions[k];
      ...
      ref = k.split(':'), _ = ref[0], region_name = ref[1];
...
var ContentFlow, FlowAPI,
  extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  hasProp = {}.hasOwnProperty;

ContentFlow = __webpack_require__(3);

FlowAPI = (function(superClass) {
  extend(FlowAPI, superClass);

  function FlowAPI(baseUrl, baseParams) {
    if (baseUrl == null) {
      baseUrl = '/';
    }
    if (baseParams == null) {
    ...
    FlowAPI.__super__.constructor.call(this, baseUrl, baseParams);
  }
...
var ContentTools, ImageUploader;

ContentTools = __webpack_require__(2);

ImageUploader = (function() {
  function ImageUploader(dialog, baseParams) {
    if (baseParams == null) {
      baseParams = {};
    }
    this.dialog = dialog;
    this.baseParams = baseParams;
    this.postTransforms = [];
    this.element = ContentEdit.Root.get().focused();
    if (this.element.type() === 'ImageFixture') {
      this.postTransforms = JSON.parse(this.element.domElement().getAttribute('data-transforms') || '[]');
    }
    dialog.addEventListener('imageuploader.cancelupload', (function(_this) {
      return function() {
        return _this.onCancelUpload();
      };
    })(this));
    ...
    dialog.addEventListener('imageuploader.fileready', (function(_this) {
      return function(ev) {
        if (ev.detail().file) {
          return _this.onFileReady(ev.detail().file);
        }
      };
    })(this));
    dialog.addEventListener('imageuploader.rotateccw', (function(_this) {
      return function(ev) {
        return _this.onRotate(-90);
      };
    })(this));
    dialog.addEventListener('imageuploader.rotatecw', (function(_this) {
      return function(ev) {
        return _this.onRotate(90);
      };
    })(this));
  ...
  ImageUploader.prototype.getTransformURL = function() {
    return '/manage/transform-asset';
  };

  ImageUploader.prototype.getUploadURL = function() {
    return '/manage/upload-asset';
  };

  ImageUploader.prototype.transform = function(transforms, callback) {
    var formData, k, ref, v, xhr;
    formData = new FormData();
    ref = this.baseParams;
    for (k in ref) {
      v = ref[k];
    ...
    xhr = new XMLHttpRequest();
    xhr.open('POST', this.getTransformURL(), true);
  ...
  ImageUploader.prototype.onCancelUpload = function() {
    if (this.xhr) {
      this.xhr.upload.removeEventListener('progress', this.xhrProgress);
      this.xhr.upload.removeEventListener('load', this.xhrComplete);
      this.xhr.abort();
    }
    return this.dialog.state('empty');
  };

  ImageUploader.prototype.onClear = function() {
    this.dialog.clear();
    return this.image = null;
  };

  ImageUploader.prototype.onFileReady = function(file) {
    ...
    this.xhr.open('POST', this.getUploadURL(), true);
    this.xhrProgress = (function(_this) {
      return function(ev) {
        return _this.dialog.progress((ev.loaded / ev.total) * 100);
      };
    })(this);
    this.xhr.upload.addEventListener('progress', this.xhrProgress);
    this.xhrComplete = (function(_this) {
      return function(ev) {
        var asset, response;
        if (parseInt(ev.target.status) === 200) {
          response = JSON.parse(ev.target.responseText);
          ...
          if (asset['type']) {
            ...
            _this.dialog.populate(_this.image.url, [_this.image.width, _this.image.height]);
          } else {
            new ContentTools.FlashUI('no');
          }
        } else {
    ...
    this.xhr.addEventListener('load', this.xhrComplete);
    return this.xhr.send(formData);
  };

  ImageUploader.prototype.onRotate = function(angle) {
    var transforms;
    ...
    return this.transform(transforms, (function(_this) {
      return function(asset) {
        var width;
        _this.image.url = asset['variations']['--draft--']['url'];
        if (_this.image.angle === 90 || (_this.image.angle = 270)) {
          _this.image.height = asset['core_meta']['image']['size'][0];
          _this.image.maxWidth = asset['core_meta']['image']['size'][1];
          _this.image.width = width = asset['core_meta']['image']['size'][1];
        } else {
          _this.image.height = asset['core_meta']['image']['size'][1];
          _this.image.maxWidth = asset['core_meta']['image']['size'][0];
          _this.image.width = width = asset['core_meta']['image']['size'][0];
        }
        return _this.dialog.populate(_this.image.url, [_this.image.width, _this.image.height]);
      };
    })(this));
  };

  ImageUploader.prototype.onSave = function() {
    var maxWidth, region, transforms;
    transforms = [];
    if (this.image.angle !== 0) {
      transforms.push({
        'id': 'image.rotate',
    ...
    region = this.dialog.cropRegion();
    maxWidth = this.image.maxWidth;
    if (region.toString() !== [0, 0, 1, 1].toString()) {
      transforms.push({
        'id': 'image.crop',
        'settings': {
    ...
    transforms = transforms.concat(this.postTransforms);
    return this.transform(transforms, (function(_this) {
      return function(asset) {
        var draft;
        draft = asset['variations']['--draft--'];
        _this.dialog.save(draft['url'], [draft['core_meta']['image']['size'][0], draft['core_meta']['image']['size'][1]], {
          'alt': '',
          'data-ce-max-width': maxWidth,
          'data-mh-asset-key': asset['key']
        });
        if (_this.element.type() === 'ImageFixture') {
          _this.element.attr('data-mh-asset-key', asset['key']);
          if (!_this.element.attr('alt')) {
            return _this.element.attr('alt', '');
          }
        }
      };
    })(this));
